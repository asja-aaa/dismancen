 ...
 */
@RestController
@RequestMapping("//house-destory")
public class HouseDestoryController {

    @Autowired
    private IHouseDestoryService service;

    @GetMapping("/inputlocalfile")
    public @ResponseBody
    String fileInputHouseDestory(@RequestParam String filePath)  {
        ...
        if(DATA_TYPE.JSON.getType().contains(filePath.substring(index+1))) {
            ...
            FileDto<HouseDestory> fileDto = JSON.parseObject(Str,new TypeReference<FileDto<HouseDestory>>(){});
...
//            }
            ...
            DisasterInfo<HouseDestory> disasterInfo = (DisasterInfo<HouseDestory>) xstream.fromXML(Str);
